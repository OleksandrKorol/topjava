...
public class JdbcMealRepositoryImpl implements MealRepository {
    ...
    public Meal save(Meal meal, int userId) {
        ...
        if (meal.isNew()) {
            ...
            int respUpdate = namedParameterJdbcTemplate.update(
                    "UPDATE meals SET id=:id, user_id=:userId, date_time=:date_time, " +
                            "description=:description, calories=:calories WHERE id=:id AND user_id=:userId", map);

            if (respUpdate == 0) return null;
        }
        return meal;
    }
